...
def _get_cutout_zoomed(bead_im, bead_labels, bead_dict):
    ...
    ll_x = int(
        np.clip(
            x - half_width - padding, 0, bead_im.shape[0] - 2 * (half_width + padding)
        )
    )
    ll_y = int(
        np.clip(
            y - half_width - padding, 0, bead_im.shape[1] - 2 * (half_width + padding)
        )
    )
...
def _get_cutout_full(bead_im, bead_labels, bead_dict):
    ...
    center = (
        np.clip(int(bead_dict["x"] * Settings.zoom_level), 0, bead_im.shape[0] - 1),
        np.clip(int(bead_dict["y"] * Settings.zoom_level), 0, bead_im.shape[1] - 1),
    )
    ...
    ll_y = int(
        np.clip(
            ...
            0,
            bead_im.shape[1] - 2 * (half_width + padding),
        )
    )
    ...
    small_width = int(np.round((2 * (half_width + padding)) / Settings.zoom_level))

    labels_small = bead_labels[
        small_x : small_x + small_width, small_y : small_y + small_width
    ]
...
def _fit_bead_zoomed(cut_collection):
    ...
    x2d = np.array([[t * 1.0] * fit_conf.shape[1] for t in range(fit_conf.shape[0])])
    y2d = np.array([np.arange(fit_conf.shape[1]) for t in range(fit_conf.shape[0])])

    pedestal = np.clip(fit_im.min().values, 0, 1e7)

    r = 0.5 * 0.5 * (fit_im.shape[0] + fit_im.shape[1])
    center = [0.5 * fit_im.shape[0], 0.5 * fit_im.shape[1]]
...
def find_beads(mos_zarr: Path):  # noqa: C901
    ...
    for this_slice in sections:
        ...
        mean_channel = (
            ...
            .mean(dim=("x", "y", "z"))
            * mos_zoom[this_slice].attrs["bscale"]
            + mos_zoom[this_slice].attrs["bzero"]
        ).values
    ...
    _str = " ".join(
        [
            "C{0:02d}: {1:.2f}".format(mos_zoom.channel.values[t], mean_per_channel[t])
        ...
        for this_optical in list(mos_full.z.values):
            ...
            im = (
                mos_zoom[this_slice].sel(
                    z=this_optical, type="mosaic", channel=channels_for_beads
                )
            ) * mos_zoom[this_slice].attrs["bscale"] + mos_zoom[this_slice].attrs[
                "bzero"
            ]

            conf = mos_zoom[this_slice].sel(
                z=this_optical, type="conf", channel=channels_for_beads
            )
            ...
            stage = np.clip((im - im_med) / im_std, -2, 2) / 2.0
            ...
            while i * (model_window - window_offset) < im_shape[0]:
                ...
                d0_corner = np.clip(
                    i * (model_window - window_offset), 0, im_shape[0] - model_window
                )
            ...
            coords = peak_local_max(distance, footprint=np.ones((5, 5)), labels=im_temp)
            ...
            logger.debug("Found {0:d} preliminary beads".format(int(labels.max())))
            ...
            full_im = (
                mos_full[this_slice].sel(
                    z=this_optical, type="mosaic", channel=channels_for_beads
                )
                * mos_full[this_slice].attrs["bscale"]
                + mos_full[this_slice].attrs["bzero"]
            )
